from __future__ import annotations

import json
import os
import re
from dataclasses import asdict, dataclass
from datetime import datetime
from pathlib import Path
from typing import Any, Dict, List, Sequence

from platform_cli.core.errors import PlatformError
from platform_cli.core.repo_roots import resolve_repo_root
from platform_cli.exec.runner import CmdResult, run_cmd
from platform_cli.manifests.orchestrate_load import load_orchestrate_json_file
from platform_cli.manifests.orchestrate_stage_load import load_stage_contract
from platform_cli.state.store import FileLock, default_state_paths
from platform_cli.tools.orchestrate_contract import load_agent_contract, runtime_branch_folder_name, slugify_branch_name
from platform_cli.tools.repo_ready_generation import current_branch_name


_BRANCHES_ROOT = Path(".ghdp/orchestrate/branches")
_SKILLS_ROOT = Path(".ghdp/skills")
_PLUGINS_ROOT = Path(".ghdp/plugins")
_STAGE_TEST_EXECUTION = "stage16_developer_test_execution"
_STAGE_BINARY_VALIDATION = "stage17_packaged_artifact_validation"
_POA_BINARY_BEGIN = "<!-- GHDP:BEGIN STAGE17_BINARY_VALIDATION -->"
_POA_BINARY_END = "<!-- GHDP:END STAGE17_BINARY_VALIDATION -->"


@dataclass
class OrchestrateBinaryValidationResult:
    repo_root: str
    branch_name: str
    branch_slug: str
    ticket_key: str
    active_run_key: str
    action: str
    status: str
    current_stage: str
    next_action: str
    branch_runtime_root: str
    validation_agent: str
    allowed_skills: List[str]
    allowed_plugins: List[str]
    package_root: str
    installed_cli_version: str
    smoke_commands: List[str]
    message: str

    def to_dict(self) -> Dict[str, Any]:
        return asdict(self)


def run_packaged_artifact_validation_stage(*, repo_root: Path | None = None) -> OrchestrateBinaryValidationResult:
    resolved_root = resolve_repo_root(repo_root)
    branch_name = current_branch_name(resolved_root)
    if not branch_name:
        raise PlatformError(
            "Could not resolve the current branch for orchestrate packaged artifact validation.",
            code="E_ORCHESTRATE_BRANCH_UNRESOLVED",
            reason="current_branch",
        )

    runtime_root = resolved_root / _BRANCHES_ROOT / runtime_branch_folder_name(resolved_root, branch_name)
    branch_state_path = runtime_root / "branch_state.json"
    if not branch_state_path.exists():
        raise PlatformError(
            "No branch runtime state exists yet. Complete the earlier orchestrator stages before packaged artifact validation begins.",
            code="E_ORCHESTRATE_RUN_MISSING",
            reason=str(branch_state_path),
        )

    branch_state = load_orchestrate_json_file(branch_state_path)
    active_run_key = str(branch_state.get("active_run_key", "")).strip()
    if not active_run_key:
        raise PlatformError(
            "Branch runtime state does not contain an active run key.",
            code="E_ORCHESTRATE_RUN_MISSING",
            reason="active_run_key",
        )

    run_root = runtime_root / "runs" / active_run_key
    stage_status_path = run_root / "stage_status.json"
    stage_status = load_orchestrate_json_file(stage_status_path)
    if str(stage_status.get(_STAGE_TEST_EXECUTION, {}).get("status", "")).strip() != "completed":
        raise PlatformError(
            "Stage 16 developer test execution must complete before Stage 17 packaged artifact validation begins.",
            code="E_ORCHESTRATE_STAGE_ORDER",
            reason=_STAGE_TEST_EXECUTION,
        )

    stage_contract = load_stage_contract(stage_id=_STAGE_BINARY_VALIDATION, repo_root=resolved_root)
    agent_contract = load_agent_contract(agent_id="binary-validation", repo_root=resolved_root)
    allowed_skills = _normalize_list(agent_contract.get("allowed_skills", []))
    allowed_plugins = _normalize_list(agent_contract.get("allowed_plugins", []))
    _assert_skill_payloads(resolved_root, allowed_skills)
    _assert_plugin_payloads(resolved_root, allowed_plugins)

    package_root = _resolve_package_root(resolved_root)
    validation = _run_packaged_validation(repo_root=resolved_root, package_root=package_root)
    version_result = validation["version"]
    status_result = validation["status"]
    smoke_commands = [
        "pipx uninstall ghdp",
        f"pipx install {package_root}",
        "ghdp --version",
        f"ghdp --json orchestrate status --repo-root {resolved_root}",
    ]

    _write_markdown(
        run_root / "binary_validation_prompt.md",
        [
            "# Stage 17 Packaged Artifact Validation Prompt",
            "",
            f"- Agent: `{agent_contract['id']}`",
            f"- Role: `{agent_contract['role']}`",
            f"- Branch: `{branch_name}`",
            f"- Ticket: `{str(branch_state.get('ticket_key', '')).strip() or '(missing)'}`",
            "",
            "## Prompt Contract",
            *[f"- {line}" for line in agent_contract.get("prompt_contract", [])],
            "",
            "## Allowed Skills",
            *[f"- `{item}`" for item in allowed_skills],
            "",
            "## Allowed Plugins",
            *[f"- `{item}`" for item in allowed_plugins],
            "",
            "## Validation Posture",
            *[f"- {line}" for line in stage_contract.get("validation_posture", [])],
            "",
        ],
    )
    _write_json(
        run_root / "binary_validation_bindings.json",
        {
            "schema_version": "1.0",
            "agent_id": agent_contract["id"],
            "allowed_skills": [
                {
                    "id": skill_id,
                    "path": str((resolved_root / _SKILLS_ROOT / skill_id / "SKILL.md").relative_to(resolved_root)).replace("\\", "/"),
                }
                for skill_id in allowed_skills
            ],
            "allowed_plugins": [
                {
                    "id": plugin_id,
                    "path": str((resolved_root / _PLUGINS_ROOT / plugin_id / "plugin.json").relative_to(resolved_root)).replace("\\", "/"),
                }
                for plugin_id in allowed_plugins
            ],
            "package_root": str(package_root),
            "smoke_commands": smoke_commands,
        },
    )
    _write_markdown(
        run_root / "artifact_validation_result.md",
        [
            "# Artifact Validation Result",
            "",
            "- Status: `completed`",
            "- Owner agent: `binary-validation`",
            f"- Package root: `{package_root}`",
            f"- Installed CLI version: `{version_result.stdout or '(unknown)'}`",
            "",
            "## Smoke Commands",
            *[f"- `{item}`" for item in smoke_commands],
            "",
            "## Installed CLI Status Output",
            "```json",
            status_result.stdout or "{}",
            "```",
            "",
        ],
    )
    _write_markdown(
        run_root / "artifact_validation_summary.md",
        [
            "# Artifact Validation Summary",
            "",
            "- Status: `completed`",
            "- Owner agent: `binary-validation`",
            f"- Installed CLI version: `{version_result.stdout or '(unknown)'}`",
            "",
            "## Ready Inputs",
            *[f"- `{item}`" for item in stage_contract.get("summary_ready_inputs", [])],
            "",
            "## Expected Next Step",
            f"- {str(stage_contract.get('summary_expected_next_step', '')).strip()}",
            "",
        ],
    )

    _upsert_stage_status(
        stage_status_path,
        stage_name=_STAGE_BINARY_VALIDATION,
        status="completed",
        owner_agent="binary-validation",
        summary="Stage 17 packaged artifact validation installed the CLI through pipx and verified the packaged command path with a live orchestrate status call.",
        artifacts=[
            "binary_validation_prompt.md",
            "binary_validation_bindings.json",
            "artifact_validation_result.md",
            "artifact_validation_summary.md",
        ],
    )
    _upsert_decisions(
        run_root / "decisions.json",
        [
            {
                "id": "stage17_packaged_artifact_validation",
                "decision": "Stage 17 validated the installed packaged CLI path before release-facing progression continued.",
                "status": "completed",
                "source": _STAGE_BINARY_VALIDATION,
            }
        ],
    )

    next_action = _stage_text(stage_contract, "next_actions", "completed")
    branch_state["status"] = "paused"
    branch_state["current_stage"] = _STAGE_BINARY_VALIDATION
    branch_state["next_action"] = next_action
    branch_state["anomaly_flag"] = False
    branch_state["last_updated_at"] = _iso_now()
    branch_state["last_updated_by"] = "binary-validation"
    _write_json(branch_state_path, branch_state)

    _update_poa(
        runtime_root / "poa.md",
        validation_agent=agent_contract["id"],
        allowed_skills=allowed_skills,
        allowed_plugins=allowed_plugins,
        package_root=package_root,
        installed_cli_version=version_result.stdout or "(unknown)",
    )
    _write_handoff(
        runtime_root / "handoff.md",
        summary=_stage_text(stage_contract, "handoff_summaries", "completed"),
        next_action=next_action,
        status="paused",
        at=_iso_now(),
    )
    _write_resume_context(
        run_root / "resume_context.md",
        active_run_key=active_run_key,
        current_stage=_STAGE_BINARY_VALIDATION,
        next_action=next_action,
        notes=_render_templates(
            stage_contract.get("resume_note_templates", []),
            validation_agent=agent_contract["id"],
            allowed_skill_count=len(allowed_skills),
            allowed_plugin_count=len(allowed_plugins),
            package_root=package_root,
            installed_cli_version=version_result.stdout or "(unknown)",
            smoke_command_count=len(smoke_commands),
        ),
    )

    return OrchestrateBinaryValidationResult(
        repo_root=str(resolved_root),
        branch_name=branch_name,
        branch_slug=slugify_branch_name(branch_name),
        ticket_key=str(branch_state.get("ticket_key", "")).strip(),
        active_run_key=active_run_key,
        action="binary_validation",
        status="paused",
        current_stage=_STAGE_BINARY_VALIDATION,
        next_action=next_action,
        branch_runtime_root=str(runtime_root),
        validation_agent=str(agent_contract["id"]),
        allowed_skills=allowed_skills,
        allowed_plugins=allowed_plugins,
        package_root=str(package_root),
        installed_cli_version=version_result.stdout or "",
        smoke_commands=smoke_commands,
        message=_stage_text(stage_contract, "messages", "completed"),
    )


def _resolve_package_root(repo_root: Path) -> Path:
    candidates = []
    if (repo_root / "platform-cli" / "pyproject.toml").exists():
        candidates.append((repo_root / "platform-cli").resolve())
    if (repo_root / "pyproject.toml").exists():
        candidates.append(repo_root.resolve())
    for child in repo_root.iterdir():
        if child.is_dir() and (child / "pyproject.toml").exists():
            candidates.append(child.resolve())
    unique = _normalize_paths(candidates)
    if len(unique) == 1:
        return unique[0]
    if not unique:
        raise PlatformError(
            "Could not resolve a package root for Stage 17 packaged artifact validation.",
            code="E_ORCHESTRATE_PACKAGE_ROOT_MISSING",
            reason=str(repo_root),
        )
    preferred = [item for item in unique if item.name == "platform-cli"]
    if len(preferred) == 1:
        return preferred[0]
    joined = ", ".join(str(item) for item in unique)
    raise PlatformError(
        f"Could not resolve a single package root for Stage 17 packaged artifact validation. Candidates: {joined}",
        code="E_ORCHESTRATE_PACKAGE_ROOT_AMBIGUOUS",
        reason=joined,
    )


def _run_packaged_validation(*, repo_root: Path, package_root: Path) -> Dict[str, CmdResult]:
    lock_path = default_state_paths().state_dir / "pipx_install.lock"
    with FileLock(lock_path, timeout_s=60.0, poll_s=0.5):
        run_cmd(["pipx", "uninstall", "ghdp"], cwd=repo_root, check=False, capture=True, timeout_s=180)
        install_result = run_cmd(["pipx", "install", "."], cwd=package_root, check=False, capture=True, timeout_s=900)
        if install_result.returncode != 0:
            raise PlatformError(
                "Stage 17 packaged artifact validation could not install the CLI through pipx.",
                code="E_ORCHESTRATE_BINARY_INSTALL_FAILED",
                reason=install_result.stderr or "pipx install failed",
            )
        env = os.environ.copy()
        version_result = run_cmd(["ghdp", "--version"], cwd=repo_root, check=False, capture=True, timeout_s=120, env=env)
        if version_result.returncode != 0:
            raise PlatformError(
                "Stage 17 packaged artifact validation installed GHDP but could not run `ghdp --version`.",
                code="E_ORCHESTRATE_BINARY_SMOKE_FAILED",
                reason=version_result.stderr or "ghdp --version failed",
            )
        status_result = run_cmd(
            ["ghdp", "--json", "orchestrate", "status", "--repo-root", str(repo_root)],
            cwd=repo_root,
            check=False,
            capture=True,
            timeout_s=180,
            env=env,
        )
        if status_result.returncode != 0:
            raise PlatformError(
                "Stage 17 packaged artifact validation installed GHDP but the packaged CLI could not run orchestrate status.",
                code="E_ORCHESTRATE_BINARY_SMOKE_FAILED",
                reason=status_result.stderr or "orchestrate status failed",
            )
    return {"version": version_result, "status": status_result}


def _normalize_paths(paths: Sequence[Path]) -> List[Path]:
    seen: set[str] = set()
    ordered: List[Path] = []
    for path in paths:
        normalized = str(path.resolve())
        if normalized in seen:
            continue
        seen.add(normalized)
        ordered.append(path.resolve())
    return ordered


def _normalize_list(items: Sequence[str]) -> List[str]:
    seen: set[str] = set()
    ordered: List[str] = []
    for item in items:
        normalized = str(item).strip()
        if not normalized or normalized in seen:
            continue
        seen.add(normalized)
        ordered.append(normalized)
    return ordered


def _assert_skill_payloads(repo_root: Path, skill_ids: Sequence[str]) -> None:
    missing = [skill_id for skill_id in skill_ids if not (repo_root / _SKILLS_ROOT / skill_id / "SKILL.md").exists()]
    if missing:
        raise PlatformError(
            "Stage 17 packaged artifact validation requires repo-level skill payloads under .ghdp/skills/<id>/SKILL.md.",
            code="E_ORCHESTRATE_SKILL_PAYLOAD_MISSING",
            reason=", ".join(missing),
        )


def _assert_plugin_payloads(repo_root: Path, plugin_ids: Sequence[str]) -> None:
    missing = [plugin_id for plugin_id in plugin_ids if not (repo_root / _PLUGINS_ROOT / plugin_id / "plugin.json").exists()]
    if missing:
        raise PlatformError(
            "Stage 17 packaged artifact validation requires repo-level plugin payloads under .ghdp/plugins/<id>/plugin.json.",
            code="E_ORCHESTRATE_PLUGIN_PAYLOAD_MISSING",
            reason=", ".join(missing),
        )


def _update_poa(
    path: Path,
    *,
    validation_agent: str,
    allowed_skills: Sequence[str],
    allowed_plugins: Sequence[str],
    package_root: Path,
    installed_cli_version: str,
) -> None:
    existing = path.read_text(encoding="utf-8") if path.exists() else ""
    managed_block = "\n".join(
        [
            _POA_BINARY_BEGIN,
            "## Stage 17 Packaged Artifact Validation",
            "",
            f"- Validation agent: `{validation_agent}`",
            f"- Allowed skill count: `{len(allowed_skills)}`",
            f"- Allowed plugin count: `{len(allowed_plugins)}`",
            f"- Package root: `{package_root}`",
            f"- Installed CLI version: `{installed_cli_version or '(unknown)'}`",
            "",
            "### Allowed Skills",
            *[f"- `{item}`" for item in allowed_skills],
            "",
            "### Allowed Plugins",
            *[f"- `{item}`" for item in allowed_plugins],
            "",
            _POA_BINARY_END,
            "",
        ]
    )
    if _POA_BINARY_BEGIN in existing and _POA_BINARY_END in existing:
        pattern = re.compile(rf"{re.escape(_POA_BINARY_BEGIN)}.*?{re.escape(_POA_BINARY_END)}\n?", re.DOTALL)
        updated = pattern.sub(lambda _match: managed_block, existing)
    else:
        updated = existing + ("\n\n" if existing and not existing.endswith("\n\n") else "") + managed_block
    path.write_text(updated, encoding="utf-8")


def _upsert_stage_status(
    path: Path,
    *,
    stage_name: str,
    status: str,
    owner_agent: str,
    summary: str,
    artifacts: Sequence[str],
) -> None:
    payload = load_orchestrate_json_file(path) if path.exists() else {}
    existing = payload.get(stage_name)
    if not isinstance(existing, dict):
        existing = {"started_at": _iso_now(), "retry_count": 0}
    if "started_at" not in existing or not str(existing.get("started_at", "")).strip():
        existing["started_at"] = _iso_now()
    existing["status"] = status
    existing["owner_agent"] = owner_agent
    existing["summary"] = summary
    existing["artifacts"] = list(artifacts)
    existing["ended_at"] = _iso_now()
    payload[stage_name] = existing
    _write_json(path, payload)


def _upsert_decisions(path: Path, decisions: Sequence[Dict[str, Any]]) -> None:
    payload = load_orchestrate_json_file(path) if path.exists() else {"schema_version": "1.0", "decisions": []}
    existing = payload.get("decisions", [])
    indexed = {str(item.get("id", "")).strip(): item for item in existing if isinstance(item, dict)}
    for decision in decisions:
        indexed[str(decision.get("id", "")).strip()] = decision
    payload["schema_version"] = str(payload.get("schema_version", "1.0")).strip() or "1.0"
    payload["decisions"] = list(indexed.values())
    _write_json(path, payload)


def _write_markdown(path: Path, lines: Sequence[str]) -> None:
    path.write_text("\n".join(lines) + "\n", encoding="utf-8")


def _write_handoff(path: Path, *, summary: str, next_action: str, status: str, at: str) -> None:
    path.write_text(
        "\n".join(
            [
                "# Handoff",
                "",
                f"- Updated at: `{at}`",
                f"- Status: {status}",
                "",
                "## Summary",
                summary,
                "",
                "## Next Steps",
                f"- {next_action}",
                "",
            ]
        ),
        encoding="utf-8",
    )


def _write_resume_context(path: Path, *, active_run_key: str, current_stage: str, next_action: str, notes: Sequence[str]) -> None:
    path.write_text(
        "\n".join(
            [
                "# Resume Context",
                "",
                f"Active run: `{active_run_key}`",
                f"Current focus: `{current_stage}`",
                f"Next action: {next_action}",
                "",
                "## Activity Log",
                *[f"- {note}" for note in notes],
                "",
            ]
        ),
        encoding="utf-8",
    )


def _stage_text(contract: Dict[str, Any], section: str, key: str) -> str:
    payload = contract.get(section, {})
    if not isinstance(payload, dict):
        return ""
    return str(payload.get(key, "")).strip()


def _render_templates(templates: Sequence[str], **context: Any) -> List[str]:
    return [str(template).format(**context) for template in templates if str(template).strip()]


def _iso_now() -> str:
    return datetime.now().astimezone().isoformat(timespec="seconds")


def _write_json(path: Path, payload: Dict[str, Any]) -> None:
    path.parent.mkdir(parents=True, exist_ok=True)
    path.write_text(json.dumps(payload, indent=2, sort_keys=True) + "\n", encoding="utf-8")
