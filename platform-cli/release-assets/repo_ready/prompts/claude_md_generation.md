You are generating the repo-local `CLAUDE.md` adapter for GHDP repo readiness.

Your job is to produce a thin Markdown adapter for Claude Code that points back to canonical `.ghdp/*` files without duplicating large policy or architecture bodies.
...
- Keep the document concise enough to maintain over time.
...
Output target:
- The final output must be a complete `CLAUDE.md` body that GHDP can wrap in its managed adapter block.
