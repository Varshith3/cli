...
def _seed_orchestrate_contract(repo_root: Path) -> None:
    ...
    agent_contracts = {
        ...
        "regression-validation": {
            ...
            "allowed_plugins": ["provider-codex", "provider-claude"],
            "produces_artifacts": ["regression_selection", "regression_results"],
            "approval_mode": "on_failure",
            "can_block": True,
            "can_retry": True,
            "prompt_contract": ["Protect already-working behavior before expanding scope."],
        },
        ...
        "binary-validation": {
            ...
            "stages_owned": ["stage17_packaged_artifact_validation"],
            "allowed_skills": ["isolated-binary-validation"],
            "allowed_plugins": ["native-memory-filesystem"],
            "produces_artifacts": ["artifact_validation_result", "packaged_cli_smoke"],
            "approval_mode": "on_failure",
            "can_block": True,
            "can_retry": True,
    ...
    (repo_root / ".ghdp" / "skills" / "manifest.json").write_text(
        json.dumps({"schema_version": "1.0", "skills": [{"id": "touched-scope-regression"}]}, indent=2) + "\n",
        encoding="utf-8",
    )
    (repo_root / ".ghdp" / "plugins" / "manifest.json").write_text(
        json.dumps({"schema_version": "1.0", "plugins": [{"id": "provider-codex"}]}, indent=2) + "\n",
        encoding="utf-8",
    )
        ...
        json.dumps(
            {
                ...
                "summary": (
                    "Objective\n"
                    "Implement the phase 1 orchestrator foundation.\n\n"
                    "Acceptance Criteria\n"
                    ...
                    "- later stages should remain resumable through repo-backed artifacts\n"
                ),
                "intent": "Implement the baseline orchestrator foundation with repo-level skills, plugins, and native memory.",
            },
            indent=2,
        )
        + "\n",
    ...
    for plugin_id in ("provider-codex", "provider-claude", "native-memory-filesystem", "github-release-gh", "jenkins-mcp", "jira-acli"):
...
def _seed_and_run_to_stage13(repo_root: Path, monkeypatch) -> None:
...
def test_regression_stage_generates_repo_backed_selection(tmp_path: Path, monkeypatch) -> None:
    repo_root = tmp_path / "repo"
    repo_root.mkdir()
    _seed_and_run_to_stage13(repo_root, monkeypatch)

    result = run_regression_stage(repo_root=repo_root)

    assert result.current_stage == "stage14_touched_scope_regression"
    assert result.regression_agent == "regression-validation"
    assert result.selected_tests
    assert "touched-scope-regression" in result.allowed_skills

    run_root = Path(result.branch_runtime_root) / "runs" / result.active_run_key
    assert (run_root / "regression_prompt.md").exists()
    assert (run_root / "regression_bindings.json").exists()
    assert (run_root / "regression_selection.md").exists()
    assert (run_root / "regression_summary.md").exists()


def test_orchestrate_regression_cli_reports_selected_tests(tmp_path: Path, monkeypatch) -> None:
    repo_root = tmp_path / "repo"
    repo_root.mkdir()
    _seed_and_run_to_stage13(repo_root, monkeypatch)

    result = runner.invoke(app, ["orchestrate", "regression", "--repo-root", str(repo_root)])

    assert result.exit_code == 0
    assert "current_stage         : stage14_touched_scope_regression" in result.output
    assert "regression_agent      : regression-validation" in result.output
    assert "selected_tests:" in result.output
