...
def _seed_orchestrate_contract(repo_root: Path) -> None:
    ...
    (repo_root / ".ghdp" / "skills" / "manifest.json").write_text(
        json.dumps({"schema_version": "1.0", "skills": [{"id": "qa-scenario-generation"}]}, indent=2) + "\n",
        encoding="utf-8",
    )
...
def _seed_and_run_to_stage12(repo_root: Path, monkeypatch) -> None:
...
def test_qa_stage_generates_repo_backed_scenarios(tmp_path: Path, monkeypatch) -> None:
    repo_root = tmp_path / "repo"
    repo_root.mkdir()
    _seed_and_run_to_stage12(repo_root, monkeypatch)

    result = run_qa_scenario_stage(repo_root=repo_root)

    assert result.current_stage == "stage13_qa_scenario_design"
    assert result.qa_agent == "qa-scenario-design"
    assert result.scenario_count >= 8
    assert "qa-scenario-generation" in result.allowed_skills

    run_root = Path(result.branch_runtime_root) / "runs" / result.active_run_key
    assert (run_root / "qa_scenario_prompt.md").exists()
    assert (run_root / "qa_scenario_bindings.json").exists()
    assert (run_root / "qa_scenario_plan.md").exists()
    assert (run_root / "qa_scenario_summary.md").exists()


def test_orchestrate_qa_cli_reports_scenario_count(tmp_path: Path, monkeypatch) -> None:
    repo_root = tmp_path / "repo"
    repo_root.mkdir()
    _seed_and_run_to_stage12(repo_root, monkeypatch)

    result = runner.invoke(app, ["orchestrate", "qa-scenarios", "--repo-root", str(repo_root)])

    assert result.exit_code == 0
    assert "current_stage         : stage13_qa_scenario_design" in result.output
    assert "qa_agent              : qa-scenario-design" in result.output
    assert "scenario_count        :" in result.output
