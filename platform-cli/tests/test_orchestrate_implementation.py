...
def test_implementation_stage_generates_stage11_packet(tmp_path: Path, monkeypatch) -> None:
    ...
    assert result.current_stage == "stage11_implementation"
    assert result.implementation_agent == "implementation"
    assert "traceability-and-resume" in result.allowed_skills
    assert "provider-codex" in result.allowed_plugins

    run_root = Path(result.branch_runtime_root) / "runs" / result.active_run_key
    assert (run_root / "implementation_prompt.md").exists()
    assert (run_root / "implementation_bindings.json").exists()
    assert (run_root / "implementation_summary.md").exists()


def test_orchestrate_implement_cli_reports_allowed_access(tmp_path: Path, monkeypatch) -> None:
    repo_root = tmp_path / "repo"
    repo_root.mkdir()
    _seed_orchestrate_contract(repo_root)
    branch_name = "feature/EPPE-7391-TECHNICAL-agentic-framework-foundation-skills-plugins-subagents-native-memory"
    ...
    run_review_layer(repo_root=repo_root)
    run_execution_prep(repo_root=repo_root)
    result = runner.invoke(app, ["orchestrate", "implement", "--repo-root", str(repo_root)])

    assert result.exit_code == 0
    assert "current_stage         : stage11_implementation" in result.output
    assert "implementation_agent  : implementation" in result.output
    assert "allowed_skills:" in result.output
