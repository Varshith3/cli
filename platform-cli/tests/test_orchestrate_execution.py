...
def _seed_orchestrate_contract(repo_root: Path) -> None:
    ...
    agent_contracts = {
        ...
        "release-prerelease": {
            ...
            "allowed_skills": ["release-and-pr", "stable-release-notes-assembly", "published-prerelease-retest"],
        ...
        "pr-external-integration": {
            ...
            "allowed_skills": ["release-and-pr", "jira-acli-integration", "pr-branch-hygiene", "pr-prerelease-commentary"],
            "allowed_plugins": ["jenkins-mcp", "jira-acli", "github-release-gh"],
            "produces_artifacts": ["pr_link", "jira_update_summary", "pr_branch_hygiene", "pr_prerelease_comment"],
        ...
        "traceability-historian": {
            ...
            "allowed_skills": ["traceability-and-resume", "audit-export-persistence", "phase-regroup-and-restart"],
            "allowed_plugins": ["native-memory-filesystem"],
        ...
        json.dumps(
            {
                "schema_version": "1.0",
                "skills": [
                    {"id": "traceability-and-resume"},
                    {"id": "published-prerelease-retest"},
                    {"id": "pr-branch-hygiene"},
                    {"id": "pr-prerelease-commentary"},
                    {"id": "audit-export-persistence"},
                    {"id": "phase-regroup-and-restart"},
                ],
            },
            indent=2,
        )
        ...
        json.dumps(
            {
                "schema_version": "1.0",
                "plugins": [
                    {"id": "native-memory-filesystem"},
                    {"id": "github-release-gh"},
                    {"id": "jira-acli"},
                    {"id": "jenkins-mcp"},
                ],
            },
    ...
    for plugin_id, executor in (
        ("native-memory-filesystem", "filesystem"),
        ("github-release-gh", "gh_cli"),
        ("jira-acli", "acli"),
        ("jenkins-mcp", "mcp"),
        ("provider-codex", "codex_cli"),
        ("provider-claude", "claude_cli"),
    ):
    ...
    (repo_root / ".ghdp" / "orchestrate" / "phases.json").write_text(
        json.dumps({"schema_version": "1.0", "single_phase_default": True, "max_impacted_areas_per_phase": 5, "max_capabilities_per_phase": 4, "force_multi_phase_keywords": ["phase"], "restart_triggers": {"too_many_impacted_areas": True, "too_many_asset_targets": True}, "restart_destinations": {"too_many_impacted_areas": "stage_c_front_door_gates", "too_many_asset_targets": "independent_asset_lifecycle"}}, indent=2) + "\n",
        encoding="utf-8",
    )
...
def test_execution_prep_generates_stage_e_artifacts(tmp_path: Path, monkeypatch) -> None:
    ...
    assert result.current_stage == "stage_e_execution_prep"
    assert "traceability-and-resume" in result.skills_bound
    assert "provider-codex" in result.plugins_bound

    run_root = Path(result.branch_runtime_root) / "runs" / result.active_run_key
    assert (run_root / "implementation_plan.md").exists()
    assert (run_root / "qa_scenario_plan.md").exists()
    assert (run_root / "regression_plan.md").exists()
    assert (run_root / "coverage_plan.md").exists()
    assert (run_root / "execution_bindings.json").exists()


def test_orchestrate_execution_prep_cli_reports_bound_skills(tmp_path: Path, monkeypatch) -> None:
    repo_root = tmp_path / "repo"
    repo_root.mkdir()
    _seed_orchestrate_contract(repo_root)
    branch_name = "feature/EPPE-7391-TECHNICAL-agentic-framework-foundation-skills-plugins-subagents-native-memory"
    ...
    run_front_door_gates(repo_root=repo_root)
    run_review_layer(repo_root=repo_root)
    result = runner.invoke(app, ["orchestrate", "execution-prep", "--repo-root", str(repo_root)])

    assert result.exit_code == 0
    assert "current_stage         : stage_e_execution_prep" in result.output
    assert "skills_bound:" in result.output
